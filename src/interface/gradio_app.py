...
def create_interface():
    ...
    with gr.Blocks(css=CUSTOM_CSS, title="LicenseWise – KBS License Advisor") as demo:
        ...
        if LOAD_ERROR:
            gr.Markdown(f"⚠️ **Warning**: Could not load license database. {LICENSES_LOADED} licenses loaded.\n\n{LOAD_ERROR}")
        else:
            gr.Markdown(f"✅ Successfully loaded **{LICENSES_LOADED} licenses** from database.")
        ...
        with gr.Tab("📋 License Recommendation"):
            ...
            gr.Markdown("Fill out the form below to get personalized license recommendations based on your project's needs.")
            ...
            with gr.Row():
                modify_library = gr.Radio(YES_NO_SKIP_CHOICES, label="Will you modify the library?", value=SKIP_VALUE)
                want_public_domain = gr.Radio(YES_NO_SKIP_CHOICES, label="Dedicate to public domain?", value=SKIP_VALUE)
                want_simple_permissive = gr.Radio(YES_NO_SKIP_CHOICES, label="Prefer simple permissive license?", value=SKIP_VALUE)
            
            with gr.Row():
                academic = gr.Radio(YES_NO_SKIP_CHOICES, label="Academic project?", value=SKIP_VALUE)
                mixed_codebase = gr.Radio(YES_NO_SKIP_CHOICES, label="Mixed open/proprietary codebase?", value=SKIP_VALUE)
                legal_recognition = gr.Radio(YES_NO_SKIP_CHOICES, label="Concerned about legal recognition?", value=SKIP_VALUE)
            
            recommend_btn = gr.Button("🔍 Get Recommendation", variant="primary", size="lg")
            recommend_output = gr.Markdown("### Results will appear here after clicking the button above")
        ...
        with gr.Tab("🔍 License Analysis"):
            ...
            gr.Markdown("Enter a license ID and answer questions about your use case to check compatibility.")
            ...
            analyze_btn = gr.Button("✅ Check Compatibility", variant="primary", size="lg")
            analyze_output = gr.Markdown("### Compatibility results will appear here after clicking the button above")
