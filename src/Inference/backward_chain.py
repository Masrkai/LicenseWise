...
def backward_chain(
    ...
    if is_eliminated:
        ...
        if conds.get("disclose_source") and facts.get("closed_source"):
            ...
            how_steps.append(
                f"{license_id} requires disclose_source=true, but you answered closed_source=true"
            )
        ...
        if conds.get("same_license") and facts.get("wants_relicense"):
            ...
            how_steps.append(
                f"{license_id} requires same_license=true, but you answered wants_relicense=true"
            )
        ...
        if facts.get("need_patent_protection") and not lims.get("patent_use"):
            how_steps.append(
                f"{license_id} offers no patent protection (patent_use=false) - consider this carefully"
            )
    ...
    relevant_warnings = []
    for step in trace:
        if step["action"] == "WARN" and any(
            pid in step.get("licenses_affected", []) for pid in possible_ids
        ):
