...
def _is_compatible(a: dict, b: dict) -> float:
    ...
    if a.get("type") == "copyleft" and b.get("type") == "copyleft":
        if a.get("conditions", {}).get("same_license") and b.get("conditions", {}).get("same_license"):
...
def make_compatibility_matrix(licenses: list[dict], max_licenses: int = 20) -> plt.Figure:
    """Heatmap with fallback for small license sets."""
    if not licenses:
        fig, ax = plt.subplots(figsize=(6, 4))
        fig.patch.set_facecolor("#1E1E2E")
        ax.set_facecolor("#1E1E2E")
        ax.text(0.5, 0.5, "No licenses loaded", ha="center", va="center", color="white", fontsize=12)
        ax.axis("off")
        return fig

    def _rank(l): return l.get("metadata", {}).get("popularity_rank") or 999
    ...
    if n < 2:
        ...
        fig.patch.set_facecolor("#1E1E2E")
        ax.set_facecolor("#1E1E2E")
        ax.text(0.5, 0.5, f"Need at least 2 licenses to build a matrix (found {n})", 
    ...
    cmap = mcolors.LinearSegmentedColormap.from_list("compat", ["#EF5350", "#FFC107", "#4CAF50"])
    ...
    for i in range(n):
        for j in range(n):
            ...
            ax.text(j, i, symbol, ha="center", va="center", fontsize=7, color=color, fontweight="bold")
    ...
    ax.set_title("License Compatibility Matrix", fontsize=14, fontweight="bold", color="white", pad=12)
...
def make_dependency_graph(licenses: list[dict]) -> plt.Figure:
    ...
    condition_nodes = ["include_copyright", "document_changes", "disclose_source", "same_license", "net_copyleft"]
    outcome_nodes = [
        "Can keep\nmodifications\nprivate",
        "Can use in\nproprietary\nproducts",
        "Can offer\nas SaaS",
    ...
    for c in condition_nodes:
        x, y = pos[c]
        ax.scatter(x, y, s=1200, color="#37474F", zorder=5, edgecolors="#78909C", linewidths=1.2, marker="D")
        ax.text(x, y, c.replace("_", "\n"), ha="center", va="center", fontsize=6.5, color="#B0BEC5", zorder=6)

    # Draw outcome nodes
    for o in outcome_nodes:
        if o in pos:
            x, y = pos[o]
            ax.scatter(x, y, s=1500, color="#263238", zorder=5, edgecolors="#546E7A", linewidths=1.2, marker="s")
            ax.text(x, y, o, ha="center", va="center", fontsize=6.5, color="#CFD8DC", zorder=6)

    # Legend
    legend_patches = [mpatches.Patch(color=c, label=t) for t, c in TYPE_COLORS.items()]
    legend_patches += [mpatches.Patch(color="#37474F", label="Condition (◆)"),
                       mpatches.Patch(color="#263238", label="Outcome (■)")]
    ax.legend(handles=legend_patches, loc="lower right", facecolor="#2D2D3F", edgecolor="#555",
              labelcolor="white", fontsize=8, framealpha=0.9)

    for label, x in [("License Types", 0.0), ("Conditions", 0.45), ("Outcomes", 0.9)]:
        ax.text(x, 2.55, label, ha="center", va="center", fontsize=10, fontweight="bold", color="#90CAF9")

    ax.set_xlim(-0.2, 1.1)
    ax.set_ylim(-0.3, 2.8)
    ax.set_title("License Knowledge Graph — Types → Conditions → Outcomes", fontsize=13, fontweight="bold", color="white", pad=10)
    fig.tight_layout()
    return fig
...
def make_statistics_charts(licenses: list[dict]) -> plt.Figure:
    ...
    wedges, _, autotexts = ax.pie(sizes, labels=None, colors=colors, autopct="%1.0f%%",
                                  startangle=140, wedgeprops=dict(width=0.55, edgecolor="#1E1E2E", linewidth=2))
    for at in autotexts:
        at.set(color="white", fontsize=8, fontweight="bold")
    ax.legend(wedges, [f"{l} ({c})" for l, c in zip(labels, sizes)], loc="lower center",
              bbox_to_anchor=(0.5, -0.18), facecolor="#2D2D3F", edgecolor="#555", labelcolor="white", fontsize=8, ncol=2)
    ax.set_title("License Type Distribution", color="white", fontsize=11, fontweight="bold", pad=8)
    ...
    ax.set_title("OSI & FSF Approval by License Type", color="white", fontsize=11, fontweight="bold")
    ...
    for l in licenses:
        cat = l.get("type", "")
        if cat not in type_perm_counts: continue
        type_totals[cat] += 1
    ...
    for i, (pl, pc) in enumerate(zip(perm_labels, perm_colors)):
        heights = [(type_perm_counts[cat][i] / type_totals[cat] * 100) if type_totals[cat] else 0 for cat in categories]
        ax.bar(x, heights, width=0.55, bottom=bottom, label=pl, color=pc, edgecolor="#1E1E2E", linewidth=0.8)
        bottom += np.array(heights)
    ax.set_xticks(x)
    ax.set_xticklabels([c.replace("_", "\n") for c in categories], fontsize=8, color="white")
    ax.set_ylabel("% of licenses granting permission", color="white", fontsize=9)
    ax.set_title("Permission Coverage per License Type", color="white", fontsize=11, fontweight="bold")
    ax.set_ylim(0, 420)
    ax.legend(facecolor="#2D2D3F", edgecolor="#555", labelcolor="white", fontsize=8, loc="upper right")

    fig.suptitle("LicenseWise – Knowledge Base Statistics", fontsize=14, fontweight="bold", color="white", y=1.01)
    fig.tight_layout()
    return fig
...
def _missing_dep_figure(package: str) -> plt.Figure:
    ...
    ax.set_facecolor("#1E1E2E")
    ax.axis("off")
    ax.text(0.5, 0.5, f"Missing dependency: {package}\nInstall with: pip install {package}",
            ha="center", va="center", fontsize=12, color="#EF5350", transform=ax.transAxes)
    return fig